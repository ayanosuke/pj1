...
#@#-----------------------------------------------------------------------------
#@# パンタスパークファンクションデコーダを作る
#@#-----------------------------------------------------------------------------

== パンタスパークファンクションデコーダを作る

前前項に紹介した@<tt>{LED}ファンクションデコーダにパンタスパークの効果を実装します。速度によってパンタスパークの点灯パターンを変えたり、点灯間隔をランダムに可変します。

「@<tt>{Arduino}で作るジオラマの仕掛けとスケッチの小技」の 6.3　ステップ、シーケンス、パターン制御で紹介している
@<tt>{SerLight}クラスをちょっと改造して使用します。
...
@<href>{https://github.com/DccShield/FunctionDecoderLEDpanta,[ FunctionDecoderLEDpantaスケッチ ]}
...
@<tt>{PantaSpark.cpp         :　}パンタスパーク点灯用クラス

@<tt>{PantaSpark.h           :　}パンタスパーク点灯用クラス用ヘッダファイル

@<tt>{FunctionDecoderLEDpanta.ino :　}パンタスパークファンクションデコーダスケッチ

=== スケッチの説明

全文を掲載するとページ数が多くなりますので前章の@<tt>{LED}ファンクションデコーダからの変更点をご紹介します。
@<tt>{FunctionDecoderLEDpanta.ino}の先頭部分に、@<tt>{SeqLight PantaLight = SeqLight(PIN_F0_F);} の行を
追加して@<tt>{SeqLight}クラスを@<tt>{PantaLight}という名前を付けて実体化（オブジェクト化）しています。
スマイルデコーダの@<tt>{D3}ポートが基板の@<tt>{PAD}に出ていますので、@<tt>{SeqLight()}のコンストラクタに
@<tt>{LED}ポート@<tt>{(PIN_F0_F)D3 PD3}を設定しています。
...
@<tt>{SeqLight}クラスを流用してパンタスパークの点灯パターンを用意しています。
光の強弱等を合わせて5パターン用意しています。
@<img>{panta1}の様に@<tt>{ptn1〜ptn5}までの点灯パターンを用意しています。単調に光らせているだけですと飽きてしまうのでこの様に
光の強弱と点灯パターンを作っています。
...
long mtbl[17][2]={       //Min, Max
                          ...
                          {  0,   0}}; // 256
//}
@<tt>{mtabl[]}は速度によって光る頻度を変えるためのテーブルになります。
受信した速度データ@<tt>{0〜255}（本スケッチ内でスケーリングした速度データです）を@<tt>{16}の範囲に分けて、
光った後の休止時間を決めるための下限と上限の時間テーブルになります。
たとえば速度が@<tt>{16}の場合パターンを光らせた後、@<tt>{300ms〜400ms}の間からランダムな数値を取得しその時間発光を止めます。
速度@<tt>{177}以上は止める時間を0にしていますので間空けずに連続に光る様になっています。

速度が遅い時は点灯間隔は広め、速度が早い時は点灯間隔の間無しという事をやっています。

//image[panta2][パンタスパーク点灯間隔][scale=1]

=== パンタスパークステートマシン

@<tt>{void SeqLight::stateCheck(uint16_t SpeedRef)}では、点灯・消灯処理、点灯パターンのランダム取得、速度による点灯間隔の制御を
行なっています。ステートマシンを表に表しました。
...
スケッチが管理している角度とサーボモータの角度が不一致になる事を防止しています。
...
6行目〜　起動時一度だけ処理させるスケッチです。@<tt>{EEPROM}に格納してあった@<tt>{STR/DIV}の情報と@<tt>{DCC}で受信した情報を
...
サーボモータファンクションデコーダと似ていますがファンクションで受信する部分をアクセサリで受信できる
ように一部修正しています。よってスケッチの説明を一部割愛しています。
...
@<tt>{FunctionDecoderMP3.ino   ：} メインスケッチです。
